...
public class AutomaticInventory extends JavaPlugin {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
		...
		if (cmd.getName().equalsIgnoreCase("debugai") && player != null) {
			...
			/*
			 * for(int i = 0; i < inventory.getSize(); i++) { ItemStack stack =
			 * inventory.getItem(i); if(stack != null)
			 * AutomaticInventory.AddLogEntry(String.valueOf(i) + " : " +
			 * stack.getType().name()); }
			 */
			...
			// create a thread to search those snapshots and create a chain of
			// quick deposit attempts
	...
	static DepositRecord depositMatching(PlayerInventory source, Inventory destination, boolean depositHotbar) {
		...
		for (int i = sourceStartIndex; i < sourceSize; i++) {
			...
			if (AutomaticInventory.instance.config_noAutoDepositIDs.contains(sourceStack.getTypeId()))
				continue;
	...
	public class FakePlayerInteractEvent extends PlayerInteractEvent {
		public FakePlayerInteractEvent(Player player, Action rightClickBlock, ItemStack itemInHand, Block clickedBlock,
				BlockFace blockFace) {
