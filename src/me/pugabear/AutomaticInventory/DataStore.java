...
public class DataStore {
	...
	private void loadMessages() {
		...
		// initialize defaults
		// this.addDefault(defaults, Messages.NoManagedWorld, "The
		// PopulationDensity plugin has not been properly configured. Please
		// update your config.yml to specify a world to manage.", null);
		this.addDefault(defaults, Messages.NoPermissionForFeature, "You don't have permission to use that feature.",
				null);
		...
		this.addDefault(defaults, Messages.InventorySortDisabled, "Stopped auto-sorting your personal inventory.",
				null);
		this.addDefault(defaults, Messages.AutoSortHelp, "Options are /AutoSort Chests and /AutoSort Inventory.", null);
		this.addDefault(defaults, Messages.AutoRefillEducation,
				"AutomaticInventory(AI) will auto-replace broken tools and depleted hotbar stacks from your inventory.",
				null);
		this.addDefault(defaults, Messages.InventorySortEducation,
				"AutomaticInventory(AI) will keep your inventory sorted.  Use /AutoSort to disable.", null);
		this.addDefault(defaults, Messages.ChestSortEducation3,
				"AutomaticInventory(AI) will sort the contents of chests you access.  Use /AutoSort to toggle.  TIP: Want some chests sorted but not others?  Chests with names including an asterisk (*) won't auto-sort.  You can rename any chest using an anvil.",
				null);
		this.addDefault(defaults, Messages.SuccessfulDeposit2, "Deposited {0} items.", null);
		this.addDefault(defaults, Messages.FailedDepositNoMatch,
				"No items deposited - none of your inventory items match items in that chest.", null);
		this.addDefault(defaults, Messages.QuickDepositAdvertisement3,
				"Want to deposit quickly from your hotbar?  Just pick a specific chest and sneak (hold shift) while hitting it.",
				null);
		...
		this.addDefault(defaults, Messages.DepositAllAdvertisement,
				"TIP: Instantly deposit all items from your inventory into all the right nearby boxes with /DepositAll!",
				null);
		...
		for (int i = 0; i < messageIDs.length; i++) {
			...
			if (messageData == null) {
				AutomaticInventory
				.AddLogEntry("Missing message for " + messageID.name() + ".  Please contact the developer.");
