...
public class AIEventHandler implements Listener {
	...
	private void tryRefillStackInHand(Player player, EquipmentSlot slot, boolean dataValueMatters) {
		...
		if (AutomaticInventory.instance.config_noAutoRefillIDs.contains(stack.getTypeId()))
			return;
		if (!dataValueMatters || stack.getAmount() == 1) {
			PlayerInventory inventory = player.getInventory();
			AutomaticInventory.instance.getServer().getScheduler().scheduleSyncDelayedTask(AutomaticInventory.instance,
					new AutoRefillHotBarTask(player, inventory, slotIndex, stack.clone(), dataValueMatters), 2L);
		}
	}
	...
	static boolean featureEnabled(Features feature, Player player) {
		...
		switch (feature) {
		case SortInventory:
			if (data.isSortInventory())
				return true;
			break;
		case SortChests:
			if (data.isSortChests())
				return true;
			break;
		case RefillStacks:
			return true;
	...
	private static boolean itemsAreSimilar(ItemStack a, ItemStack b, boolean dataValueMatters) {
		if (a.getType() == b.getType() && (!dataValueMatters || a.getData().getData() == b.getData().getData())) {
			if (a.containsEnchantment(Enchantment.LOOT_BONUS_BLOCKS) || a.containsEnchantment(Enchantment.SILK_TOUCH)
	...
	class AutoRefillHotBarTask implements Runnable {
		...
		public AutoRefillHotBarTask(Player player, PlayerInventory targetInventory, int slotToRefill,
				ItemStack stackToReplace, boolean dataValueMatters) {
			this.player = player;
	...
	public void onBlockDamage(BlockDamageEvent event) {
		...
		@SuppressWarnings("deprecation")
		PlayerInteractEvent fakeEvent = AutomaticInventory.instance.new FakePlayerInteractEvent(player,
				Action.RIGHT_CLICK_BLOCK, player.getItemInHand(), clickedBlock, BlockFace.EAST);
		...
		// send confirmation message to player with counts deposited. if none
		// deposited, give instructions on how to set up the chest.
		if (deposits.destinationFull && deposits.totalItems == 0) {
			...
			AutomaticInventory.sendMessage(player, TextMode.Success, Messages.SuccessfulDeposit2,
					String.valueOf(deposits.totalItems));

			// make a note that quick deposit was used so that player will not
			// be bothered with advertisement messages again.
			PlayerData playerData = PlayerData.FromPlayer(player);
	...
	public void onInventoryOpen(InventoryOpenEvent event) {
		...
		if (!player.isSneaking() && featureEnabled(Features.SortChests, player)) {
			...
			if (!isSortableChestInventory(topInventory))
				return;
	...
	public void onInventoryClose(InventoryCloseEvent event) {
		...
		if (player.getGameMode() != GameMode.CREATIVE && Math.random() < .1 && !playerData.isGotDepositAllInfo()
				&& featureEnabled(Features.DepositAll, player)) {
			Inventory topInventory = event.getView().getTopInventory();
		...
		for (int i = 1; i < stacks.size(); i++) {
			...
			if (prevStack.isSimilar(thisStack)) {
				if (prevStack.getAmount() < prevStack.getMaxStackSize()) {
					int moveCount = Math.min(prevStack.getMaxStackSize() - prevStack.getAmount(),
							thisStack.getAmount());
