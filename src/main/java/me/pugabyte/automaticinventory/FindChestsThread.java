...
public class FindChestsThread extends Thread {
	...
	public FindChestsThread(World world, ChunkSnapshot[][] snapshots, int minY, int maxY, int startX, int startY,
			int startZ, Player player) {
		this.world = world;
	...
	public void run() {
		...
		while (!leftToVisit.isEmpty()) {
			...
			if (isChestID(typeID)) {
				int overTypeID = this
						.getTypeID(new Vector(current.getBlockX(), current.getBlockY() + 1, current.getBlockZ()));
	...
	private Location makeLocation(Vector location) {
		return new Location(this.world, this.smallestChunk.getX() * 16 + location.getBlockX(), location.getBlockY(),
	...
	private boolean isChestID(int id) {
		return (id == 54 || id == 146 || (id > 218 && id < 235));
	}
	...
	class QuickDepositChain implements Runnable {
		...
		QuickDepositChain(Queue<Location> remainingChestLocations, DepositRecord runningDepositRecord, Player player,
		...
		public void run() {
			...
			if (chestLocation == null) {
				AutomaticInventory.sendMessage(this.player, TextMode.Success, Messages.SuccessfulDepositAll2,
						String.valueOf(this.runningDepositRecord.totalItems));
				PlayerData playerData = PlayerData.FromPlayer(player);
				if (Math.random() < .1 && !playerData.isGotQuickDepositInfo()
						&& AIEventHandler.featureEnabled(Features.QuickDeposit, player)) {
				...
				PlayerInteractEvent fakeEvent = AutomaticInventory.instance.new FakePlayerInteractEvent(player,
						Action.RIGHT_CLICK_BLOCK, player.getInventory().getItemInMainHand(), block, BlockFace.UP);
				...
				if (!fakeEvent.isCancelled()) {
					...
					if (state instanceof InventoryHolder) {
						...
						if (!this.respectExclusions || AIEventHandler.isSortableChestInventory(chestInventory)) {
							...
							DepositRecord deposits = AutomaticInventory.depositMatching(playerInventory, chestInventory,
									false);
				...
				QuickDepositChain chain = new QuickDepositChain(this.remainingChestLocations, this.runningDepositRecord,
						this.player, this.respectExclusions);
				Bukkit.getScheduler().runTaskLater(AutomaticInventory.instance, chain, 1L);
			}
		}
	}
